...
Console.WriteLine();//used for formatting gives a blank space

// Filter the list to create a new list that only has 
//  place names that start with the word "The".
List<string> theNames = names.Where(n => n.StartsWith("The")).ToList();//.where is the same as .filter

Console.WriteLine("\"The\" Place Names");// "\"the escape code to tell what's NOT code
